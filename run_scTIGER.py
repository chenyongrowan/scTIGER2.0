...
parser.add_argument("-od", "override_downsample", dest="override", action = "store_true", help="Overiding downsample of 200 cells.")
...
if (args.ctrl=="No"):
    ...
    if(args.override == True or len(ctrlW_data.columns) < 200): ##did columns not rows
        ctrlW = ctrlW_data
    else:
...
while args.start <= 3:
    if version == 1.0:
        ...
        if args.start == 1: 
            ...
            sig_nb, sig_parab, sig_avg = sd.GRAPH(args.outputDir, geneList, args.runs, args.timeDelay, args.alpha, hold)
            with open('../commandDetails.txt', 'a') as file:
